...
async function main() {
  console.log('');
  console.log('🚀 ══════════════════════════════════════════');
  console.log('   Traffy Fondue → Supabase ETL Pipeline');
  console.log('   ══════════════════════════════════════════');
  console.log('   Batches:  ' + MAX_BATCHES + ' × ' + BATCH_SIZE + ' = ' + (MAX_BATCHES * BATCH_SIZE).toLocaleString() + ' records max');
  ...
  for (let offset = 0; offset < MAX_BATCHES * BATCH_SIZE; offset += BATCH_SIZE) {
    batchNum++;
    const pct = Math.round((batchNum / MAX_BATCHES) * 100);
    process.stdout.write('📦 [' + pct + '%] Batch ' + batchNum + '/' + MAX_BATCHES + ' (offset=' + offset + ')... ');
    ...
    try {
      ...
      try {
        ...
        console.log('❌ JSON parse error (API timeout at high offset) — stopping.');
        break;
      }
      ...
      for (const r of data.results) {
        ...
        // Auto-correct flipped coords
        if (lat > 80 && lat < 120 && lon > 10 && lon < 20) {
          const tmp = lon; lon = lat; lat = tmp;
        }

        const district = extractDistrict(r.address);
        const rawType = r.problem_type_abdul && r.problem_type_abdul[0] && r.problem_type_abdul[0] !== '';
        const problemType = rawType ? r.problem_type_abdul[0] : classifyProblem(r.description);
      ...
      // Upsert to Supabase in chunks of 500 (Supabase limit)
      for (let i = 0; i < rows.length; i += 500) {
        const chunk = rows.slice(i, i + 500);
        const { error } = await supabase
          .from('traffy_complaints')
          .upsert(chunk, { onConflict: 'ticket_id', ignoreDuplicates: false });
      ...
      console.log('✅ ' + rows.length + ' rows (total: ' + totalInserted.toLocaleString() + ', ' + elapsed + 's elapsed)');
  ...
  console.log('');
  console.log('═══════════════════════════════════════');
  console.log('✅ ETL Complete in ' + totalTime + ' seconds!');
  ...
  console.log('📊 Total records in Supabase: ' + (count ? count.toLocaleString() : 'unknown'));
  console.log('');
}
